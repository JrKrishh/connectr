...
- `src/ui/page.ts` — the whole dashboard as one embedded HTML page (no build step, no CDN). **Every backslash in the client JS must be doubled** — the file is a template literal, and a single one is eaten before the browser sees it (see lesson f29). Shell-aware: when `window.connectr` exists (the Electron preload), it adds a Projects button and the Ctrl+K project switcher; in a browser it stays a plain page; board columns, agents, memory, claims, run tails, add + confirm-dispatch
