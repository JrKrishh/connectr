...
- `src/cli/index.ts` — `connectr` CLI: serve, init, new (project creation: folder + PLAN.md + suggested orchestra, scoped wiring, seeds the decompose ticket), task add (auto/manual route + model, validates --tool), run (parallel dispatch = the host loop, injects planFile), status, board, doctor, dash
