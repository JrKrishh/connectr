...
- `src/host.ts` — shared host actions used by CLI run, TUI dash and web UI: `addTaskFromInput` (parse + route + create), `planIntent` (park an intent as a planner ticket routed to the project's default/reasoning tool), `planOpenTickets` (route-and-copy open set, include/exclude), `launchPlanned`
- `src/learn.ts` — outcome-learned routing: `categoryOf(title, desc)` matches the title first and only widens to the description when the title says nothing; `learnRoutes` scores tools per category from closed tickets (completions = wins, takeovers + routing-misses = losses, Laplace-smoothed); `resolveToolSmart` overrides the static rule only at 3+ outcomes **and** once the rule's own tool has been tried there (otherwise "never tried" would read as "worse"); surfaced via `connectr routes` and `routedTo.via/reason`
